...
export async function getAccounts(): Promise<FinanceAccount[]> {
    ...
    // 2. Para cada cuenta, traemos con precisión de secuencia la transacción más antigua y más reciente
    const enrichedAccounts = await Promise.all(
        accounts.map(async (acc) => {
            // Transacción MÁS RECIENTE con saldo (Techo / Saldo Actual)
            const { data: latestTxs } = await supabase
                .from('finance_transactions')
                .select('date, amount, bank_balance, import_sequence')
            ...
            // Transacción MÁS ANTIGUA con saldo (Cimiento / Saldo Inicial)
            ...
            if (latestTx && oldestTx) {
                ...
                // ESCENARIO A: Cuenta con historial de saldos bancarios
                calculatedCurrentBalance = Number(latestTx.bank_balance);
                
                // Saldo inicial de la historia = (bank_balance de la 1ª tx) - (importe de la 1ª tx)
                const oldestBalCentimos = Math.round(Number(oldestTx.bank_balance || 0) * 100);
                const oldestAmountCentimos = Math.round(Number(oldestTx.amount || 0) * 100);
                calculatedInitialBalance = (oldestBalCentimos - oldestAmountCentimos) / 100;
            } else {
                ...
                calculatedCurrentBalance = sumData?.reduce((sum, t) => sum + Number(t.amount || 0), 0) ?? 0;
...
export async function getImportBatchDetail(id: string): Promise<{
    ...
    const { data: rawBatch, error: batchError } = await supabase
        ...
        .select(`
            ...
            import_date,
            account_id,
            finance_accounts!finance_importers_account_id_fkey (
                name,
                color_theme,
                avatar_letter
            )
    ...
    if (batchError) {
        console.error('❌ Error al consultar finance_importers:', batchError.message);
        return null;
    }

    if (!rawBatch) {
        console.warn('⚠️ Lote no encontrado con ID:', cleanId);
        return null;
    }

    // 2. Transacciones del Lote (Puras, ordenadas por import_sequence y fecha)
    const { data: rawTransactions, error: txError } = await supabase
        ...
        .select(`
            ...
            created_at
        `)
        .eq('importer_id', cleanId)
        .order('import_sequence', { ascending: true })
        .order('date', { ascending: true });

    if (txError) {
        console.error('❌ Error al consultar finance_transactions:', txError.message);
    }
    ...
    // 4. Mapeo estructurado alineado a las columnas exactas de la BBDD
    const accountData = (rawBatch as any).finance_accounts;
    ...
    const batch: BatchDetailLog = {
        ...
        account_name: accountData?.name || 'Cuenta no disponible',
        account_color: accountData?.color_theme || '#6366f1',
        account_letter: accountData?.avatar_letter || accountData?.name?.charAt(0).toUpperCase() || 'C',
        oldest_date: oldestDate,
        newest_date: newestDate,
    };
